...
// Optional Query Params: ?date=YYYY-MM-DD&start_time=10:00&end_time=12:00
router.get('/', async (req, res) => {
  try {
    ...
    if (date && start_time && end_time) {
      // Return seats marked with availability status for the given time slot
    ...
    // Default: Return all seats for the space
...
// POST /api/spaces/:spaceId/seats - Add seat (Admin only)
...
router.put('/seats/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const { seat_number, has_power_outlet } = req.body;
...
router.delete('/seats/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;
